...
def clamp_score(score: float) -> float:
    """Ensure score is strictly between 0 and 1 (exclusive) — 0.0 and 1.0 not allowed."""
...
def run_task(task: dict) -> float:
    print(f"\n{'='*55}")
    print(f"[START] task={task['id']}  ({task['num_emails']} emails)")
    print(f"{'='*55}")
    ...
    while obs.get("email_id") != "DONE":
        ...
        print(f"\n[STEP] step={steps+1} subject={subj}...")
        ...
        # Clamp per-step reward to safe range
        reward = clamp_score((result.get("reward", 0.0) + 1.0) / 2.0)
        total_reward += reward
        steps += 1
        gt = result.get("info", {}).get("ground_truth", {})
        print(f"[STEP] step={steps} reward={reward:.4f}  gt={gt}")

        if result.get("done"):
            break
        obs = result["observation"]

    # Normalize raw average from [-1,1] to (0,1) strictly
    raw_avg = total_reward / steps if steps > 0 else 0.0
    normalized = clamp_score(raw_avg)

    print(f"\n[END] task={task['id']} score={normalized:.4f} steps={steps}")
    return normalized
